...
require_once('../include/conn.php');
   ...
   function add_user()
   {
		...
		if (isset($_POST['is_pasangan'])) {
			$is_pasangan = 0;
			$_POST['nama_pasangan'] = "-";
			$_POST['pendidikan_pasangan'] = "99";
			$_POST['pekerjaan_pasangan'] = "99";
		}
		if (isset($_POST['is_anak'])) {
			$is_anak = 0;
		}
    	...
    	*/
		...
		if (	empty($_POST['uuid_bidan']) ||
				...
				*/
				...
				exit('Ada data yang belum lengkap !');
		}
		...
		if ($stmt = $con->prepare('SELECT nik FROM user WHERE nik = ?')) {
			...
			if ($stmt->num_rows > 0) {
				...
				if ($stmt = $con->prepare('INSERT INTO user (uuid , uuid_bidan, fullname ,nik, dob, phone, email, password, status) VALUES (?,?,?,?,?,?,?,?,?)')) {
					...
					if ($stmt = $con->prepare('INSERT INTO user_detail (uuid_user, nama_pasangan, pendidikan, pendidikan_pasangan, alamat, pekerjaan, pekerjaan_pasangan, asuransi, anak_pria_hidup, anak_wanita_hidup, tahun_anak_terakhir, bulan_anak_terakhir, status_kb, kb_terakhir, status, is_pasangan, is_anak) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')) {
						$stmt->bind_param('sssssssssssssssss', $uuid, $_POST['nama_pasangan'], $_POST['pendidikan'], $_POST['pendidikan_pasangan'], $_POST['alamat'], $_POST['pekerjaan'], $_POST['pekerjaan_pasangan'], $_POST['asuransi'], $_POST['anak_pria_hidup'], $_POST['anak_wanita_hidup'], $_POST['tahun_anak_terakhir'], $_POST['bulan_anak_terakhir'], $_POST['status_kb'], $_POST['kb_terakhir'], $isActive, $is_pasangan, $is_anak);
						$stmt->execute();
						$stmt->close();
